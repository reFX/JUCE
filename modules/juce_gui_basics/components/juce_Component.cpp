...
public:
    ...
    [[nodiscard]] auto pushComponent (Component& component)
    {
        ...
        return ScopeGuard { [this, pos]
        {
            ...
            // We don't need to restore the component, nothing should be trying
            // to access it again after this guard object is destroyed.
    ...
    {
        // Either the guard object returned by pushComponent() has been destroyed
        // too early, or this method is being called before a call to pushComponent().
        jassert (currentComponent != nullptr);
        const auto pos = currentComponent->getPosition();
        const auto bounds = getNonObscuredBoundsForCurrentComponent<ObscuredBy::allButChildren> (g.getClipBounds() - pos);
    ...
    Rectangle<int> getCurrentComponentPaintBounds (const juce::Graphics& g)
    {
        ...
        // too early, or this method is being called before a call to pushComponent().
        jassert (currentComponent != nullptr);
        return getNonObscuredBoundsForCurrentComponent<ObscuredBy::childrenOnly> (g.getClipBounds());
    }
...
private:
    ...
    Rectangle<int> getNonObscuredBoundsForCurrentComponent (Rectangle<int> clipBounds) const
    {
        ...
        const auto obscureWith = [&] (const OpaqueComponentInfo& opaqueComponentInfo)
        {
        ...
        if constexpr (obscuredBy == ObscuredBy::childrenOnly)
        {
            for (auto i = currentPosition; i < opaqueComponents.size(); ++i)
            {
                ...
                obscureWith (opaqueComponentInfo);
    ...
    {
        return detail::ComponentHelpers::isVisible (c)
...
struct ComponentTests  : public UnitTest
{
    ...
    void runTest() override
    {
        ...
        beginTest ("Areas of an opaque component outside its parent will not be considered opaque");
        {
            ...
            expectEquals (parent->numPaintCalls, 1);
            expect (parent->lastClipBounds == Rectangle<int> { 0, 0, 50, 100 });
            expectEquals (child1->numPaintCalls, 0);
            expectEquals (child2->numPaintCalls, 1);
            expect (child2->lastClipBounds == Rectangle<int> { 50, 0, 50, 100 });
        }
    }
};
